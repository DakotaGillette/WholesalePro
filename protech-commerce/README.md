...
**Messaging** (its own top-level wp-admin menu) has seven items: **Home** (setup checklist, counts, latest sends, "Run automations now"), **Emails** (tabs: Sent, Automatic, Templates; "Add new email" opens Compose), **Automations** (multi-step flows), **Forms**, **Contacts**, **Log** and **Settings** (tabs: General, Sending, Compliance). It sends email and SMS to wholesale customers, through Brevo (email falls back to the site's own WooCommerce mailer if Brevo isn't connected; SMS has no fallback). Nothing is ever sent from an admin or checkout request. Everything queues through the message log and delivers via Action Scheduler.
