...
**Messaging** (its own top-level wp-admin menu) has seven items: **Home** (setup checklist, counts, latest sends, "Run automations now"), **Emails** (tabs: Newsletters, Automatic, Templates; "Add new email" starts the new-email flow), **Automations** (multi-step flows), **Forms**, **Contacts**, **Log** and **Settings** (tabs: General, Sending, Compliance). It sends email and SMS to wholesale customers, through Brevo (email falls back to the site's own WooCommerce mailer if Brevo isn't connected; SMS has no fallback). Nothing is ever sent from an admin or checkout request. Everything queues through the message log and delivers via Action Scheduler.
...
- **New email** (3.9.0, "Add new email"): four steps, like MailPoet. **Type**: Email, Text message (the older one-form Compose, locked to SMS) or Automatic email (the lifecycle and order emails). **Template**: a starter, one of your saved templates, or an email you sent before; the email gets its own copy, so editing it never changes the template. **Design**: the template editor, with the subject and preview text on top; the draft saves itself a moment after each change. **Send**: who gets it (all customers, a tier, recent or lapsed buyers, buyers of a product, never ordered, or customers ticked on the Customers tab), a live count of who will get it and who is left out, the service-email switch, the sender (from name, from email on the Settings domain, reply-to), and Schedule it (a date and time on the site's clock), then "Save as draft and close", Send or Schedule. Send preview and Save as template are on the Design step's top bar. Drafts and scheduled emails wait under Emails, Newsletters; a scheduled one can be unscheduled until it sends.
...
- **Compliance** (Settings → Compliance): see the next section.
